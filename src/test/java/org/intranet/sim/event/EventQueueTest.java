...
public class EventQueueTest {
	/**
	 * Tries to test if two simultaneous events can interfere. Repeats the test many
	 * times, to increase changes to show the bug
	 *
	 * @throws Throwable
	 */
	@Test
	 ...
	 *
	 * <p>
	 * This tries to call {@link EventQueue#processEventsUpTo(long)} and at the same
	 * time {@link EventQueue#removeEvent(Event)}. What should never happen is that
	 * the event is executed while it already was removed from the queue.
	 *
	 * <p>
	 ...
	 * {@link GOALController}). however ArrivalEvent is private so we use some other
	 * event instead.
	 *
	 * @throws Throwable
	 */
	public void testMultiThreadingOnce() throws Throwable {
		...
		final Callable<Integer> removeTask = () -> {
			eQ.removeEvent(event);
			event.disable();
			return 0;
		};

		final Callable<Integer> timerTask = () -> {
